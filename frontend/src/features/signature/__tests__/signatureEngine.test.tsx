...
import { verifyPin, formatSignatureTimestamp, generateSecureHash, createSignatureBlock, verifySignatureIntegrity } from "../signatureEngine";
...
describe("createSignatureBlock", () => {
  test("generates correct block properties for HR manager approval", async () => {
    const block = await createSignatureBlock(
      "u-hr",
      "Priya Sharma",
      "TKT-1001",
      "hr_approval",
    ...
    expect(block.signedBy).toBe("Priya Sharma");
    expect(block.role).toBe("HR Manager");
    expect(block.userId).toBe("u-hr");
    expect(block.purpose).toBe("hr_approval");
    expect(block.ticketId).toBe("TKT-1001");
    expect(block.signatureImage).toBe("data:image/png;base64,mock");
    expect(block.hash).toBeDefined();
    expect(block.deviceHint).toBeDefined();
    expect(block.signedAt).toBeDefined();
  });
});

describe("verifySignatureIntegrity", () => {
  test("returns true for a valid non-tampered signature block", async () => {
    const block = await createSignatureBlock(
      "u-hr",
      "Priya Sharma",
      "TKT-1001",
    ...
    const isValid = await verifySignatureIntegrity(block);
    expect(isValid).toBe(true);
  });

  test("returns false for a tampered signature block", async () => {
    const block = await createSignatureBlock(
      "u-hr",
      "Priya Sharma",
      "TKT-1001",
    ...
    // Tamper the ticket ID
    const tamperedBlock = {
      ...block,
      ticketId: "TKT-9999"
    };
    
    const isValid = await verifySignatureIntegrity(tamperedBlock);
