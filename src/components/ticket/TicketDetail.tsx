...
export default function TicketDetail({ ticketId, onClose }: TicketDetailProps) {
  ...
  const downloadReq = async () => {
    ...
    setPdfLoading(true);
    try {
      await generateAndDownloadPdf(ticket, user || undefined);
    } finally {
      setPdfLoading(false);
    }
  };
  ...
  const resubmit = () => {
    setStatus(ticket.id, "pending_hr", {
      comment: { userId: user!.id, role: "helpdesk", text: "Resubmitted after addressing feedback." },
    });
  ...
  const markDone = () => {
    ...
    setStatus(ticket.id, "inspection_pending", {
      comment: { userId: user!.id, role: "helpdesk", text: "Work marked complete. Ready for inspection." },
    });
  ...
  const inspectionPass = () => {
    updateTicket(ticket.id, {
      inspection: { passed: true, notes: inspectionNotes || "Verified.", signedByHr: false, signedByAdmin: false },
    });
    setInspectionNotes("");
  };
        ...
        <div className="space-y-4">
          <PinThenDrawSignature
            ...
            onSigned={async (block) => {
              const updatedTicket = {
                ...ticket,
                signatures: { ...ticket.signatures, adminApproval: block },
              };
              setStatus(ticket.id, "work_in_progress", {
                comment: { userId: user!.id, role: "admin", text: "Approved by Admin." },
                patch: { signatures: { adminApproval: block } },
              });
              const dataUrl = await generateAndDownloadPdf(updatedTicket, user || undefined);
              addPdf(ticket.id, { name: `Requirement_${ticket.id}.pdf`, type: "requirement", dataUrl, at: new Date().toISOString() });
          ...
          {!ticket.assignee ? (
            <div className="grid grid-cols-2 gap-2">
              <div>
                ...
                <Input value={assignName} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setAssignName(e.target.value)} />
              </div>
              <div>
                <Label>Department</Label>
                <Input value={assignDept} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setAssignDept(e.target.value)} />
              </div>
        ...
        <div className="space-y-4">
          ...
          <PinThenDrawSignature
            ...
            onSigned={(block) => {
              updateTicket(ticket.id, {
                inspection: {
                  ...(ticket.inspection || { passed: true, notes: "", signedByHr: false, signedByAdmin: false }),
                  signedByHr: true,
                },
              ...
              <PinThenDrawSignature
                ...
                onSigned={async (block) => {
                  ...
                  const updatedTicket = {
                    ...
                    signatures: { ...ticket.signatures, adminPayment: block },
                  };
                  setStatus(ticket.id, "payment_pending", {
                    comment: { userId: user!.id, role: "admin", text: "Inspection co-signed by Admin. Ready for payment." },
                    patch: { inspection: updatedInspection, signatures: { adminPayment: block } },
                  });
                  const dataUrl = await generateAndDownloadPdf(updatedTicket, user || undefined);
      ...
      return (
        <div className="space-y-4">
          <div className="rounded-md bg-white/[0.06] border border-white/10 p-3 text-sm text-white/80">
            Amount to release: <b className="text-emerald-400">{fmtMoney(ticket.estimatedCost)}</b>
          </div>
          {!hasSigned ? (
            <PinThenDrawSignature
              userId={user!.id}
              ...
              <Button
                ...
                onClick={async () => {
                  ...
                  setStatus(ticket.id, "closed", {
                    comment: { userId: user!.id, role: "admin", text: `Payment of ${fmtMoney(payment.amount)} released. Ticket closed.` },
                    patch: { payment },
                  });
                  const dataUrl = await generateAndDownloadPdf(updatedTicket, user || undefined);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            ...
            {docView ? (
              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  ...
                  <Info
                    label="Assignee"
                    value={ticket.assignee ? `${ticket.assignee.name} (${ticket.assignee.department})` : "—"}
                  />
                  <Info
                    label="Inspection"
                    value={ticket.inspection ? (ticket.inspection.passed ? "Passed" : "Failed") : "—"}
                  />
                </div>
                ...
                <div>
                  <div className="text-xs uppercase tracking-wider text-white/40 mb-2">Documents</div>
                  <div className="flex flex-wrap gap-2">
                    {ticket.pdfs.length === 0 && (
                      <span className="text-xs text-white/40 self-center">No PDFs generated yet.</span>
                    )}
                    {ticket.pdfs.map((p, i) => (
                      <a
                        key={i}
          ...
          <div className="space-y-3">
            <div className="rounded-lg border border-white/10 bg-white/[0.06] backdrop-blur-sm p-3">
              <div className="text-xs uppercase tracking-wider text-white/50 mb-2">Available Actions</div>
              {renderActions()}
            </div>

            {ticket.signatures && Object.keys(ticket.signatures).length > 0 && (
              <div className="rounded-lg border border-white/10 bg-white/[0.06] backdrop-blur-sm p-3">
                <div className="text-xs uppercase tracking-wider text-white/50 mb-3">Signatures on file</div>
                <div className="space-y-3">
                  {ticket.signatures.hrApproval && (
                    <SignatureCard number={1} label="HR Approval" accent="text-emerald-400" block={ticket.signatures.hrApproval} />
                  )}
                  {ticket.signatures.adminApproval && (
                    <SignatureCard number={2} label="Admin Approval" accent="text-blue-400" block={ticket.signatures.adminApproval} />
                  )}
                  {ticket.signatures.hrInspection && (
                    <SignatureCard number={3} label="HR Inspection" accent="text-violet-400" block={ticket.signatures.hrInspection} />
                  )}
                  {ticket.signatures.adminPayment && (
                    <SignatureCard number={4} label="Admin Payment Auth" accent="text-amber-400" block={ticket.signatures.adminPayment} />
                  )}
                </div>
              </div>
            )}
  ...
  return (
    <div className="rounded-md border border-border overflow-hidden">
      ...
      <div className="bg-white h-12 flex items-center justify-center px-2">
        <img src={block.signatureImage} alt={label} className="max-h-10 max-w-full object-contain" />
      </div>
      <div className="px-2 py-1.5 space-y-0.5">
        ...
        <div className="text-[9px] font-mono text-white/30">Hash: {block.hash}</div>
        <div className="flex items-center gap-1 pt-0.5">
          <div className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
          <span className="text-[9px] text-emerald-500 font-semibold">PIN Verified · Digitally Signed</span>
        </div>
      </div>
    </div>
  );
